...
| `missing-permissions.spec.ts` | Permissions a sign-in lacks: a scope unticked at consent is listed in words behind a disclosure naming the count ("2 missing permissions") with "Sign in again" (disabled while the sign-in runs); dismiss keeps it away for the browser session (a day at most); a denied consent keeps the session; signing in again replaces and revokes the grant without reloading the page. A VIEW member who unticks a scope their role holds is told about that one only, never about the ones the role lacks. |
...
`TOLGEE_URL` server skips them with the reason. The probe also compares `scopes_supported` with the scopes the
extension asks for: on a server that does not know one of them the OAuth specs cannot sign in for it or untick it on
the consent screen, so they skip naming the missing scopes instead of failing opaquely.
