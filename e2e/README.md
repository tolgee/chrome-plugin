...
| Spec                          | What it covers                                                                                                                                                                                   |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `detection.spec.ts`           | What the popup makes of the page: project detected, page still reloading, no Tolgee at all, no content script (about:blank), another tab's handshakes, a legacy SDK, an SDK without the proxied-request protocol (no OAuth sign-in offered), two Tolgee instances (iframe). |
...
| `oauth-session.spec.ts`       | OAuth session states: account name, session revoked on the server (dialog says "not signed in", popup offers "Sign in again"), page declaring a project the session cannot reach, sign out clearing every tab, consent denied, an expired token refreshed by the worker before it sends. |
